...
public class Intermedio extends AppCompatActivity {
    ...
    Button buttonplano;
    private RecyclerView recycerViewIntermedio;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recycerViewIntermedio = findViewById(R.id.recycerViewIntermedio);
                ...
                try {
                    SimpleResponse simpleResponse =
                            Repository.getINSTANCE(getApplication()).
                                    acaoPlanos(BASE_URL, 2, loginResponse.getUtilizador().getEmail(), s, loginResponse.getToken());
    ...
    private void getExercicios(){
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    PlanoResponse planoResponse = Repository.getINSTANCE(getApplication()).planoResponse(BASE_URL,2);
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            CursosAdapter cursosAdapter = new CursosAdapter(Intermedio.this, Intermedio.this);
                            recycerViewIntermedio.setLayoutManager(new LinearLayoutManager(Intermedio.this)); //RecyclerView
                            recycerViewIntermedio.setAdapter(cursosAdapter);
