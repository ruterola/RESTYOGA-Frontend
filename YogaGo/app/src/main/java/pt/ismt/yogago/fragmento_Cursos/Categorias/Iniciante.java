...
public class Iniciante extends AppCompatActivity {
    ...
    private RecyclerView exerciciosIniciante;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate( savedInstanceState );
        setContentView( R.layout.activity_iniciante );
        exerciciosIniciante = findViewById(R.id.exerciciosIniciante);
        getExercicios();
        ...
        buttonplano.setOnClickListener( new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                else if (buttonplano.getText().equals("Terminar plano")) //BOTAO PARA TERMINAR PLANO
                {
                    ...
                    acaoPlano(ACAO.TERMINAR);
                    buttonplano.setBackground(background);
                }
            }
        } );
    }
    private void acaoPlano(ACAO acao){
                ...
                try {
                    SimpleResponse simpleResponse =
                            Repository.getINSTANCE(getApplication()).
                                    acaoPlanos(BASE_URL, 1, loginResponse.getUtilizador().getEmail(), s, loginResponse.getToken());
    ...
    private void getExercicios(){
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    PlanoResponse planoResponse = Repository.getINSTANCE(getApplication()).planoResponse(BASE_URL,1);
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            CursosAdapter cursosAdapter = new CursosAdapter(Iniciante.this, Iniciante.this);
                            exerciciosIniciante.setLayoutManager(new LinearLayoutManager(Iniciante.this)); //RecyclerView
                            exerciciosIniciante.setAdapter(cursosAdapter);
